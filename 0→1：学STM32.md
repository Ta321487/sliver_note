...
### 10. 中断管理 (NVIC)
- **抢占优先级 (Preemption Priority)**：决定了“谁能打断谁”。数字越小，优先级越高（0 是最高）。如果 A 优先级高于 B，A 可以中断 B 的执行。
- **响应优先级 (Subpriority)**：当两个中断同时发生时，谁先排队执行。
### 11. 边沿检测 (Edge Detection)

- **上升沿**：信号从 0 变到 1 的瞬间（接通电源）。
- **下降沿**：信号从 1 变到 0 的瞬间（接地）。
- **信号整形**：由 GPIO 内部的**施密特触发器**完成，它保证了只有电平跨越特定阈值时才会触发边沿检测，过滤微小波动。
### 12. 中断标志位 (Pending Bit)
- 硬件触发中断后会锁存一个标志。在处理完中断任务后，必须**清除标志位**（通过 `HAL_GPIO_EXTI_IRQHandler` 自动完成），否则 CPU 会认为中断一直在发生，从而反复跳入中断函数。
...
![](assets/0→1：学STM32/file-20260127235341528.png)![配置寄存器](assets/0→1：学STM32/file-20260127235539214.png)
...
- 实践发现仍存在小问题：存在按键按下后，绿色小灯并没有反转的情况（硬件消抖有些捉襟见肘；措施：使用软件消抖）按键按下，或者抬起时的抖动都会有下降沿，从而触发中断。![](assets/0→1：学STM32/file-20260128230436746.png)
>	按键按下时，电压并不是完美下降，而是在高低阈值之间疯狂跳动。如果跳动过快，施密特触发器可能在极短时间内产生了多次中断请求，或者因为电平在阈值边缘停留时间太短，硬件根本没识别到一个清晰的“下降沿”。
- 先延时10ms，等待抖动过去`HAL_Delay(10);`
- 10ms后，再判断当前KEY1的电平是否为低电平，判断按键是因为按下的下降沿触发，还是抬起时抖动中的下降沿触发，并在确认是按键按下后，翻转绿色小灯的电平
	```c
	if(HAL_GPIO_ReadPin(KEY1_GPIO_Port, KEY1_Pin) == GPIO_PIN_RESET){
		HAL_GPIO_TogglePin(GREEN_LED_GPIO_Port, GREEN_LED_Pin);
		}
	```
- 再次编译，更大的bug出现了：绿色小灯没亮，红色小灯也没有闪烁
- 优先级问题：`HAL_Delay()`依赖`System tick timer`（系统滴答）的中断，该中断可提供1ms的时钟基准，但该中断的优先级比我们触发的中断的优先级**低**，这就导致了不能在中断处理函数执行时执行，也就导致了`HAL_Delay()`函数不能正常执行。
- 解决优先级问题：让`System tick timer`的优先级数字小于`EXTI15_10`即可
![](assets/0→1：学STM32/file-20260128232231721.png)
==在中断中加入`HAL_Delay()`是不被推荐的做法！！！==
原因：需尽可能保证中断任务尽快完成，以将中断带来的影响降到最低
```mermaid
  graph TD
...
![](assets/0→1：学STM32/file-20260131230141226.png)外部电平信号进入到输入驱动器后，首先路过上拉下拉电阻，然后经过施密特触发器转换，最后抵达输入数据寄存器/片上外设。电平信号会抵达上图的结构。上图被称为外部中断/事件控制器。
这种结构，在STM32F1系列芯片有19个，这19个外部中断控制器共用一套寄存器，每个外部中断都对应其中一组线路，即外部中断线。外部中断线的前16个，也就是EXTI0~EXTI15，对应和编号相同的GPIO口（PA0、PB0、PC0、PD0……进入EXTI0，PA1、PB1、PC1、PD1进入EXTI1……以此类推，PA15、PB15、PC15、PD15进入到EXTI15）
事件（事件屏蔽寄存器$\rightarrow$脉冲发射器）：与中断类似，中断会抵达处理器 中调用代码处理；事件会抵达相应外设
边沿检测电路：用于检测输入的电平信号有没有发生高低电平的转换（有没有出现上升沿/下降沿），根据上升沿触发选择寄存器和下降沿触发选择寄存器的配置来决定是否向后输出高电平信号，即[[assets/0→1：学STM32/file-20260127235539214.png|配置上升沿触发选择寄存器和下降沿触发选择寄存器]]
例如：给PB12配置下降沿触发中断，上升沿触发选择寄存器的第12位关闭为0，下降沿触发选择寄存器的第12位开启为1![](assets/0→1：学STM32/file-20260206225326921.png)。边沿检测电路检测到了下降沿时，会向后传递高电平信号，经过**或门**抵达请求挂起寄存器（或门：两个输入端有一个输入为1则输出为1）。请求挂起寄存器接收到高电平信号会将对应位置置1![](assets/0→1：学STM32/file-20260304201547229.png).请求挂起寄存器又会将此位输出到**与门**（与门：两个输出都是1才是1，否则为0）。中断屏蔽寄存器1或0决定了中断控制器输出1还是0（CubeMX已经帮忙了），最后到达NVIC（嵌套向量中断控制器）
NVIC存有中断向量表
...
| EXTI5_10 | EXTI15_10_IRQHandler |
EXTI12会找到EXTI5_10，执行EXTI15_10_IRQHandler函数
NVIC一直检测某个中断线是否处于激活状态，当中断处理函数运行完成后，若依旧检测到中断处于激活状态，就再次运行处理函数。如果想要只执行一遍而不是无限重复，就需要将请求挂起寄存器对应位清除为0（CubeMX自动处理：`HAL_GPIO_EXTI_IRQHandler(KEY1_Pin);`）
```c
KEY1_Pin = GPIO_PIN_12  → EXTI12
```

中断向量表还有优先级，优先级又细分为抢占优先级和响应优先级

## 优先级

>抢占优先级、响应优先级：数字越小越优先

以下两种情况会判断中断优先级：①当两个中断几乎同时发生，先判断抢占优先级（假如A和B两个中断，A抢占优先级更高，则A先执行）；若抢占优先级相同，再比较响应优先级（响应优先级也相同，则按照中断向量表的顺序决定）②某中断正在执行，另一中断突然发生，只比较二者抢占优先级（若某个中断的抢占优先级弱于前一个中断或者与该中断相同时，则只能等待另一个中断完成再执行该中断）
存储：使用4个二进制位存储中断优先级信息。可以选择4位中哪几位用来设置抢占优先级，几位设置响应优先级![](assets/0→1：学STM32/file-20260304211053310.png)（默认为4位都设置抢占优先级）。这时候每个中断的抢占优先级可以设置为0~15，响应优先级只能设置为0）