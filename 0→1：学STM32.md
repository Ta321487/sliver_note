...
- **识别方法**：需查阅芯片 Datasheet 的引脚定义表，查看 `I/O Level` 栏位是否标有 **FT** 字样。
...
- **上保护 (VDD 侧)**：当引脚电压 $V_{in} > V_{DD}$（且非 FT 引脚）时导通，将过高电压引入电源。
    
- **下保护 (VSS 侧)**：当引脚电压 $V_{in} < V_{SS}$（出现负压）时导通，将电流引入地，保护内部电路。
...
因芯片内部电路承受的电压有限，从I/O引脚（实物图中的”铜丝“）向里（左侧）有两个保护二极管，当瞬间电压波动进入I/O引脚时，电压大于3.3V（记作$V_{DD}$，即电源正极）时，上方的保护二极管导通，将电压引入电源，由电源吸收；电压小于3.3V（VSS\[0V\]，即电源负极）时，下方保护二极管接通，电压进入GND。（*这个结构可以短暂地域，若较长时间接入较高的电压，则芯片也会损坏。）*
有一部分的I/O口可以做到”5V容忍[4. 引脚耐压与保护 (Pin Tolerance)](#4.%20引脚耐压与保护%20(Pin%20Tolerance))”（可承受5V电压作为高电平输入）
