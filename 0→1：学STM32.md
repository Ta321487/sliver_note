...
例：I/O口接了普通小灯，小灯另一端为GND，小灯工作电压为3.3V，可以使用推挽输出，在此模式下P-MOS和N-MOS协同工作，使用`HAL_GPIO_WritePin`函数控制此I/O口输出高电平，函数内部对寄存器数据发生了改变，导致P-MOS激活，N_MOS关闭![](assets/0→1：学STM32/file-20260127000410352.png)，$V_{DD}$与I/