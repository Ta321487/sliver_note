...
例如：给PB12配置下降沿触发中断，上升沿触发选择寄存器的第12位关闭为0，下降沿触发选择寄存器的第12位开启为1![](assets/0→1：学STM32/file-20260206225326921.png)。边沿检测电路检测到了下降沿时，会向后传递高电平信号，经过**或门**抵达请求挂起寄存器（或门：两个输入端有一个输入为1则输出为1）。请求挂起寄存器接收到高电平信号会将对应位置置1![](assets/0→1：学STM32/file-20260304201547229.png).