...
边沿检测电路：用于检测输入的电平信号有没有发生高低电平的转换（有没有出现上升沿/下降沿），根据上升沿触发选择寄存器和下降沿触发选择寄存器的配置来决定是否向后输出高电平信号，即配置如图所示的寄存器[file-20260127235539214](assets/0→1：学STM32/file-20260127235539214.png)
